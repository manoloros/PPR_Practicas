...
void equilibrado_carga(tPila *pila, bool *fin, tNodo *solucion)
{
	...
	if (pila->vacia()) {
			...
			while (pila->vacia() && !*fin) {
				...
				MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &estadoMensaje);

				switch (estadoMensaje.MPI_TAG) {
					case PETICION : 
						{
							int solicitante;
							MPI_Recv(&solicitante, 1, MPI_INT, estadoMensaje.MPI_SOURCE, estadoMensaje.MPI_TAG, comunicadorCarga, &estadoMensaje);	
							...
							if (solicitante == rank) {
								...
								if (token_presente) {
									...
									MPI_Send(&color_token, 1, MPI_INT, (rank-1+size) % size, TOKEN, comunicadorCarga);
									color = BLANCO;
					...
					case NODOS :
						{
							...
							int emisor = estadoMensaje.MPI_SOURCE;
							tPila nodos;

							MPI_Recv(&nodos.tope, 1, MPI_INT, emisor, NODOS, comunicadorCarga, &estadoMensaje);
							MPI_Recv(&nodos.nodos[0], nodos.tope, MPI_INT, emisor, NODOS, comunicadorCarga, &estadoMensaje);

							tNodo nodo;
							bool vacia = !nodos.pop(nodo);
							while (!vacia) {
								pila->push(nodo);
								vacia = !nodos.pop(nodo);
							}
						}
						break;
					case TOKEN :
						{
							MPI_Recv(&color_token, 1, MPI_INT, estadoMensaje.MPI_SOURCE, estadoMensaje.MPI_TAG, comunicadorCarga, &estadoMensaje);
							token_presente = true;

							if (estado == PASIVO) {
								if (rank == 0 && color == BLANCO && color_token == BLANCO) {
									MPI_Send(&solucion->datos[0], 2 * NCIUDADES, MPI_INT, (rank+1) % size, FIN, comunicadorCarga);
								} else {
					...
					case FIN :
						{
							tNodo otraSolucion;
							MPI_Recv(&otraSolucion.datos[0], 2 * NCIUDADES, MPI_INT, estadoMensaje.MPI_SOURCE, estadoMensaje.MPI_TAG, comunicadorCarga, &estadoMensaje);

							if (rank != 0)
								if (solucion->ci() != 0 && (solucion->ci() < otraSolucion.ci() || otraSolucion.ci() == 0))
									MPI_Send(&solucion->datos[0], 2 * NCIUDADES, MPI_INT, (rank+1) % size, FIN, comunicadorCarga);
								else
									MPI_Send(&otraSolucion.datos[0], 2 * NCIUDADES, MPI_INT, (rank+1) % size, FIN, comunicadorCarga);
							else 
								for (int i=0; i< 2 * NCIUDADES; i++)
									solucion->datos[i] = otraSolucion.datos[i];
	...
	if (!*fin) {
		...
		int encontrado;
		MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &encontrado, &estadoMensaje);
				...
				case PETICION :
					{
						...
						tPila nodos;
						if (pila->divide(nodos)) {
							MPI_Send(&nodos.tope, 1, MPI_INT, solicitante, NODOS, comunicadorCarga);
							MPI_Send(&nodos.nodos[0], nodos.tope, MPI_INT, solicitante, NODOS, comunicadorCarga);
...
void difusion_cota_superior(int &U, bool &nueva_U)
{
	...
	if (difundir_cs_local && !pendiente_retorno_cs) {
		MPI_Send(&U, 1, MPI_INT, (rank+1) % size, rank, comunicadorCota);						
		pendiente_retorno_cs = true;
		difundir_cs_local = false;
	}

	int encontrado;
	MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCota, &encontrado, &estadoMensaje);	

	while (encontrado > 0) {
		int U_nuevo;

		MPI_Recv(&U_nuevo, 1, MPI_INT, (rank-1+size) % size, estadoMensaje.MPI_TAG, comunicadorCota, &estadoMensaje);
		...
		if (estadoMensaje.MPI_TAG == rank && difundir_cs_local) {
			MPI_Send(&U, 1, MPI_INT, (rank+1) % size, rank, comunicadorCota);						
			pendiente_retorno_cs = true;
			difundir_cs_local = false;
		} else if (estadoMensaje.MPI_TAG == rank && !difundir_cs_local)
			pendiente_retorno_cs = false;
		else 
			MPI_Send(&U, 1, MPI_INT, (rank+1) % size, estadoMensaje.MPI_TAG, comunicadorCota);	
...
void inicializacion_variables_comunicacion()
{
	if (rank == 0)
		token_presente = true;
	else
